...
<script type="text/javascript">
	...
	$(document.body).on('click', '.show_more', function(e) {
		console.log('show more ',$(this).attr('data-nextoffset'));
		let nxtOffset = $(this).attr('data-nextoffset');
		let searchValue = $(".searchValue").val();
		doAjax("POST", searchValue, 'searchData', limit, nxtOffset);
		e.preventDefault();
	});

	function doAjax(type, data = '', method = '', limit, offset=0) {
		$(".showhide").remove();
	    $.ajax({
	  		url: "process1.php",
	  		type: type,
	...
	function viewMore(prevOffset=0) {
		let newCounter = (prevOffset === undefined || prevOffset == 0)? parseInt(0)+parseInt(limit) : parseInt(prevOffset)+parseInt(limit)+1;
		console.log('newCounter ', newCounter); 
		let html = '<div class="text-center showhide"><a href="javascript:void(0)" style="text-decoration:none" class="show_more" data-nextOffset='+newCounter+'>View More</a></div>';
		$('#tableContent').after(html);
	}
</script>
</body>
</html>